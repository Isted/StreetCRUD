...
- **[Package]**: Name of the package for the generated code. If more than one package is required, two separate files will need to be processed by Street CRUD.
...
- **[file name]**: A file name such as user.go can be added to the right of [file name] which will cause the code-generation file to be named user.go. If no name is given, default naming based on struct name will be used. If multiple struct definitions use the same value for [file name], code will be generated to the same file and not generated in seperate files.
...
- StreetCRUD does not make sure that struct variables and columns are unique. Entering identical names in the file to be processed will cause and error. This issue will be addressed in the future.
